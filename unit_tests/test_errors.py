"""
Тесты для проверки обработки ошибок и граничных случаев.

Включают:
- Обработку невалидных форматов
- Обработку повреждённых данных
- Проверку размеров и граничных условий
"""
...
def test_invalid_endpoint(client):
    """Сценарий 4.4: Обработка ошибок - вызов несуществующего эндпоинта."""
...
def test_large_image_rejection(client):
    """Сценарий 4.7: Обработка ошибок - попытка загрузить слишком тяжелое изображение."""
...
def test_corrupted_image(client):
    ...
    corrupted_data = io.BytesIO(b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x01\x00\x48")
